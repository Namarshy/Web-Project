...
export class InventoryComponent {
  ...
  getInventoryDetails() {
    let apiUrl = "https://localhost:7219/api/Inventory";
  ...
  onSubmit(): void {
    ...
    if (this.disableProductIdInput == true) {
      this.httpclient.put(apiUrl, this.inventoryData, httpOptions).subscribe({
        ...
        complete: () => {
          alert("Form updated successfully: " + JSON.stringify(this.inventoryData));
          this.getInventoryDetails();
        }
      });
    }
  ...
  openConfirmDialoge(productId: number) {
  ...
  deleteInventory(): void {
    ...
    let apiUrl = "https://localhost:7219/api/Inventory?productId=" + this.productIdToDelete;
